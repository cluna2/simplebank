...
func TestCreateTransferAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Return(db.Account{}, db.ErrRecordNotFound)
				store.EXPECT().
					TransferTx(gomock.Any(), gomock.Any()).
					Times(0)
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Any()).Times(1).Return(account1, nil)
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account2.ID)).Times(1).Return(account2, sql.ErrConnDone)
				store.EXPECT().
					TransferTx(gomock.Any(), gomock.Any()).
					Times(0)
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// create new mock db store and embed stubs
